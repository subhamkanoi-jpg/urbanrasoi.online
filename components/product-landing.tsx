          ...
          <ol className="mt-9 grid gap-4 md:grid-cols-3">
            {product.steps.map((step, index) => (
              <Reveal key={step.title} delay={index * 70}>
                <li className="h-full rounded-2xl bg-card p-5 md:p-7">
                  <span className="font-serif text-2xl text-terracotta/30 md:text-3xl">0{index + 1}</span>
                  <h3 className="mt-4 font-serif text-xl font-semibold text-ink md:mt-8">{step.title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-ink-soft md:mt-3 md:text-base">{step.detail}</p>
                </li>
              </Reveal>
            ))}
          </ol>
